...
function run-autorest($src) {
  ...
  $txt = "$autorest --version:c:\work\2019\autorest.megarepo\autorest --pipeline-model:v3 --input-file:$swaggerRoot$src --output-folder:$outputFolder --title:$name $root/modelerfour/test/scenarios/test-configuration.md $args" 
  write-host -fore GREEN "`n--------------------------------------------------------`nGenerating [$name]`n--------------------------------------------------------`n"
  echo $txt
  & $autorest "--version:c:\work\2019\autorest.megarepo\autorest" "--pipeline-model:v3" "--input-file:$swaggerRoot$src" "--output-folder:$outputFolder" "--clear-output-folder" "--title:$name" "$root/modelerfour/test/scenarios/test-configuration.md" "--deduplicate-inline-models" $args
  $rc = $LastExitCode
  if( $rc -gt 0 ) {
...
autorest-beta --version:c:\work\2019\autorest.megarepo\autorest --pipeline-model:v3 --input-file:https://github.com/Azure/azure-rest-api-specs/blob/master/specification/storage/data-plane/Microsoft.StorageDataLake/stable/2019-10-31/DataLakeStorage.json  --output-folder:C:\work\2019\autorest.megarepo\autorest.modelerfour\modelerfour\test\datalake-storage --verbose --debug --no-network-check "$root/modelerfour/test/scenarios/test-configuration.md"
